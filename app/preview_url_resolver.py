...
def get_preview_url(
    ...
    print(f"✅ Using preview URL: {url}", flush=True)
    return url
...
def wait_for_preview_ready(
    ...
    if timeout <= 0:
        print("⏭️ Skipping preview ready check (timeout is 0)", flush=True)
        return True
    ...
    while time.monotonic() < deadline:
        try:
            ...
            if time.monotonic() >= next_log:
                print(f"⏳ Waiting for preview... ({e!r})", flush=True)
                next_log = time.monotonic() + 30
        time.sleep(interval)

    print(f"❌ Preview not ready after {timeout}s: {url}", flush=True)
    return False
